...
public class StringProblems {
	...
	public String reverseVowels(String s) {
		...
		while (l < h) {
			...
			if (l < h && arr[l] != arr[h]) Utils.swap(arr, l, h);
			l++;
			h--;
		}
	...
	public boolean rotateString(String A, String B) {
		...
		for (int i = 0; i < B.length(); i++) {
			...
			int j = i;
			while (j < B.length() && B.charAt(j) != start) j++;

			if (j >= B.length()) return false;

			for (int n = 0; n < B.length(); n++) {
				if (A.charAt(n) != B.charAt((n + j) % B.length())) break;
	...
	public String convert(String s, int numRows) {
		...
		for (int i = 0; i < numRows; i++) {
			...
			step2 = 2 * i; // 0, 2, 4,...
			pos = i;
			if (pos < n) sb.append(s.charAt(pos));
			while (true) {
				pos += step1;
				if (pos >= n) break;
	...
	public int makeAnagram(String a, String b) {
		...
		for (int i = 0; i < 26; i++) {
			if (hash[i] != 0) count += Math.abs(hash[i]);
		}
	...
	static String twoStrings(String s1, String s2) {
		...
		for (int i = 0; i < s2.length(); i++)
			if (hash[s2.charAt(i) - 'a'] != 0) return "YES";
	...
	public void ransomNote1(String[] magazine, String[] note) {
		...
		for (String s : note) {
			...
			if (counter == 0) magazineMap.remove(s);
			else magazineMap.put(s, counter);
		}
